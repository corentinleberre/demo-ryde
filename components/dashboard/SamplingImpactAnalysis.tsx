...
export function SamplingImpactAnalysis() {
  ...
  const regionArray = Object.values(regionData).map((region: any) => ({
    ...region,
    revenueGrowth:
      ((region.afterRevenue - region.beforeRevenue) / region.beforeRevenue) *
      100,
    unitsGrowth:
      ((region.afterUnits - region.beforeUnits) / region.beforeUnits) * 100,
    avgROI:
      ((region.afterRevenue - region.beforeRevenue) /
        (region.totalSamples * 0.5)) *
      100, // Assuming $0.50 per sample cost
  }));
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Before vs After performance analysis of sampling campaigns
          </p>
        </div>
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-2">
            ...
            <Select
              value={viewMode}
              onValueChange={(value) =>
                setViewMode(value as "region" | "store")
              }
            >
        ...
        <Card className="p-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">
                {storeData
                  .reduce((sum, store) => sum + store.samplesDistributed, 0)
                  .toLocaleString()}
              </p>
                ...
                {formatPercent(
                  storeData.reduce(
                    (sum, store) =>
                      sum +
                      calculateImpact(
                        store.beforeSales.revenue,
                        store.afterSales.revenue
                      ),
                    0
                  ) / storeData.length
                )}
        ...
        <Card className="p-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">
                {Math.round(
                  regionArray.reduce((sum, region) => sum + region.avgROI, 0) /
                    regionArray.length
                )}
                %
              </p>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        ...
                        {region.totalSamples.toLocaleString()} samples
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-semibold text-gray-900">
                      {formatCurrency(region.beforeRevenue)}
                    </p>
                  </div>
                  <div>
                    <p className="text-gray-600">After Revenue</p>
                    <p className="font-semibold text-gray-900">
                      {formatCurrency(region.afterRevenue)}
                    </p>
                  </div>
                  <div>
                    <p className="text-gray-600">Before Units</p>
                    <p className="font-semibold text-gray-900">
                      {formatUnits(region.beforeUnits)}
                    </p>
                  </div>
                  <div>
                    <p className="text-gray-600">After Units</p>
                    <p className="font-semibold text-gray-900">
                      {formatUnits(region.afterUnits)}
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {store.location} •{" "}
                          {store.samplesDistributed.toLocaleString()} samples
                        </p>
                      </div>
                    </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                            <div>
                              ...
                              <p className="text-xs text-gray-600">
                                {store.afterSales.period}
                              </p>
                            </div>
                            <div className="text-right">
                              <p className="font-semibold text-gray-900">
                                {formatCurrency(store.afterSales.revenue)}
                              </p>
                              <p className="text-sm text-gray-600">
                                {formatCurrency(store.beforeSales.revenue)} →{" "}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                            ...
                            <div className="text-right">
                              <p className="font-semibold text-gray-900">
                                {store.afterSales.units}
                              </p>
                              <p className="text-sm text-gray-600">
                                {store.beforeSales.units} →{" "}
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                            ...
                            <div className="text-right">
                              <p className="font-semibold text-gray-900">
                                {store.afterSales.vpo}
                              </p>
                              <p className="text-sm text-gray-600">
                                {store.beforeSales.vpo} →{" "}
                              </p>
                            </div>
                          </div>
                        </div>
