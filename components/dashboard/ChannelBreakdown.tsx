...
export function ChannelBreakdown() {
  const channelData = [
    { name: 'Circana Accounts', value: 45, revenue: 1845000, change: 3.2, color: '#64748b' },
    { name: 'Amazon', value: 25, revenue: 1025000, change: 12.5, color: '#475569' },
    { name: 'TikTok Shop', value: 15, revenue: 615000, change: 28.7, color: '#6b7280' },
    { name: 'Direct Accounts', value: 15, revenue: 615000, change: -2.1, color: '#9ca3af' }
  ];
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          <div className="space-y-1">
            ...
            <p className={`flex items-center ${data.change >= 0 ? 'text-slate-600' : 'text-slate-500'}`}>
              {data.change >= 0 ? 
  ...
  const renderCustomLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent }: any) => {
    ...
    if (percent > 0.05) { // Only show label if slice is bigger than 5%
      return (
        <text 
          x={x} 
          y={y} 
            ...
            <div key={channel.name} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">${(channel.revenue / 1000000).toFixed(1)}M</p>
                </div>
              </div>
              
              <div className="flex items-center space-x-2">
                <span className="text-lg font-semibold text-gray-900">{channel.value}%</span>
                <Badge variant={channel.change >= 0 ? 'default' : 'destructive'} className="text-xs">
                  {channel.change >= 0 ? 
                    <TrendingUp className="h-3 w-3 mr-1" /> : 
                    <TrendingDown className="h-3 w-3 mr-1" />
                  }
